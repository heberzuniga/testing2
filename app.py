...
project_root = os.path.dirname(_file_)
...
if choice == "Introducción":
    ...
    if st.button("▶ Ejecutar pruebas unitarias"):
        import unittest
    ...
    if st.button("▶ Ejecutar pruebas de integración"):
        import unittest
        loader = unittest.TestLoader()
...
''', language="python")
                ...
                st.error(f"Excepción: {type(e)._name_}: {e}")
                st.code(f'''import unittest
from src.calculator import {mode}
class TestAuto(unittest.TestCase):
    def test_{mode}_raises(self):
        with self.assertRaises({type(e)._name_}):
            {mode}({a}, {b})
''', language="python")
    else:
...
''', language="python")
    ...
    st.markdown("Hecho con ❤ para demostrar buenas prácticas de testing en Python + Streamlit.")